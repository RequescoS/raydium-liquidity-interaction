...
#[derive(Accounts)]
pub struct AddAndRemoveLiquidity<'info> {
...
pub fn add_and_remove_liquidity(
    ...
    let cpi_accounts_deposit = cpi::accounts::Deposit {
    ...
    let cpi_accounts_withdraw = cpi::accounts::Withdraw {
        owner: ctx.accounts.owner.to_account_info(),
        authority: ctx.accounts.authority.to_account_info(),
        pool_state: ctx.accounts.pool_state.to_account_info(),
        owner_lp_token: ctx.accounts.owner_lp_token.to_account_info(),
    ...
    let cpi_context = CpiContext::new(
        ctx.accounts.cp_swap_program.to_account_info(),
        cpi_accounts_withdraw,
    );
    cpi::withdraw(cpi_context, lp_token_amount, 0, 0) // Set to 0 in order to prevent slippage errors
}
